...
- 提交 `8e6eb0d` 的提交信息记录：六份 Dockerfile 因 build 阶段只 COPY 部分模块而父 pom 声明全部模块，`Child module ... does not exist` → **此前一次都没构建成功过**；修复后仅 leaderboard 一份实跑 build+run，其余 5 份同批修正未逐个重跑。`ci.yml` 全文无任何 docker 步骤（实测）。`docker-compose.services.yml:22-183` 已为 6 个服务定义 `context: .` + 各自 `dockerfile:`，即门槛可直接复用 compose 的构建定义。
  **【本条证据已订正】**原写"`config -q` 在 HEAD 实测退出码 0"——那是在**有 `.env` 的开发机**上量的：`docker-compose.services.yml:26/54/84/…` 为 6 个服务都声明了 `env_file: [.env]`，而 `.env` 按约定不入库，干净检出（CI）下该步骤以 `env file ... not found` 失败。首跑证据见 CI run `35570779585`（build job 正是红在这一步，其后的镜像构建与扩围自检被 skip）。修法为 CI 自备占位 `.env`（`cp scripts/verify/env.example .env`），本地判据改为"无 `.env` 时 rc=1、放占位后 rc=0"两条都实测。这正是本提案第 7 条发现要堵的同一类错误：机器态被当成仓库态陈述。
...
- `ci.yml` 的构建步骤改为调用该脚本的 online 模式；新增四步：先 `cp scripts/verify/env.example .env` 放一份占位（compose 六个服务都声明 `env_file: [.env]`，干净检出下没有它连解析都过不去）、compose 多文件 `config -q`、复用 compose 定义构建 1 份代表服务镜像（`docker compose ... build <service>`，不手写 `docker build -f`）、web 侧显式提供 pnpm 版本后执行 `pnpm install --frozen-lockfile` + `type-check` + `build`。
