...
骨架基线已可编译、可一键启动，但**没有任何业务逻辑**——提交一条运动记录不会产生任何判定。本变更落地系统的核心灵魂：R1-R4 规则链 + 预处理漂移过滤 + 审核状态机，让「提交→校验→判定→（驳回后）申诉→终判」闭环真正跑通。这是整个反作弊项目的主线，也是面试弹药 A（算法）与 B（状态机/一致性）的代码载体。
...
- **跨库最终一致**（record_db ↔ verify_db）→ 缓解：MQ 事件 + 状态机中间态 + 幂等重试，不引入 Seata（弹药 B）。
