...
这是校验精度的实质缺口，也是深入追问「阈值怎么定」「不同运动怎么处理」时的软肋。
...
- **阈值分类型后每个类型如何定值** → 缓解：RUNNING 沿用现有 5.5；CYCLING 按骑行合理速度（如 15m/s）；各类型阈值写入配置说明依据，设计取舍可讲「per-sport-type 标定」。
...
- 技术亮点：为什么 per-sport-type 阈值、阈值标定来源（真实数据分位数）、未知类型怎么保守处理。