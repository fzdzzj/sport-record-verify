...
引入 OSM 道路拓扑匹配后，校验引擎从「规则链」升级为「规则链 + 空间真实性双保险」：R5 离路规则把轨迹点吸附到真实路网，计算「偏离路网的距离/占比」，偏离过大即判为可疑。这是技术天花板最高的一项（执行计划 P2 重度项），也是「为什么校验引擎不只有 if-else，还要空间计算」的最强论据。
...
- **OSM 路网数据获取与导入成本**（下载 PBF、空间库建表、坐标系）→ 缓解：选单个城市切片（如上海）降低体积；导入脚本幂等可重跑；设计上可讲「全国路网分片/瓦片」。
- **匹配算法正确性与性能**（HMM 复杂度、大数据量轨迹）→ 缓解：先落「最近边投影 + 偏离度」跑通闭环，HMM 作为进阶优化（离线预计算 + 空间索引）；匹配限制在固定采样点数防超时。
...
- 这是「规则链 → 规则链 + 空间真实性」的质变，不是简单加一条 if；技术亮点：为什么离线匹配、为什么 HMM、为什么 PostGIS、空间索引原理、离路阈值的工程化（per-road-type 差异）。