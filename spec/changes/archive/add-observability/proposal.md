...
项目已完成 5 个功能模块与压测，但「生产化质感」还缺关键一环：服务只有 health/info 两个 Actuator 端点，**没有任何指标采集、可视化面板、告警**。面试谈到「监控告警」目前只能讲设计（执行计划 P2 明确列为可选，进度超前来做）。落地 Prometheus + Grafana 后，项目的运维完整度大幅提升，也让压测报告（add-load-test-report）里的性能数据有了**持续观测的载体**，而不是一次性快照。
