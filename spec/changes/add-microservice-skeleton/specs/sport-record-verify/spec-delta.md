...
本文件包含对 `spec/specs/sport-record-verify/spec.md` 的规范变更（首个能力域基线，全部为新增）。
...
- 本变更仅覆盖「代码骨架」；校验引擎、好友、点赞、排行榜等业务需求在后续变更中分别以 ADDED 需求落地。
- 版本矩阵具体值见 proposal.md「What Changes」与 ADR，规范层仅约束「集中锁定 + 兼容验证」。
