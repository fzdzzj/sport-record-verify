...
项目目前所有服务**无任何认证鉴权**——6+ 处代码注释明确写着「骨架无认证鉴权，userId 由调用方显式携带，接 JWT 后改从 token 解析」。这意味着任何调用方可以伪造任意 userId，越权访问他人记录、点赞、好友、榜单。这是安全边界的硬缺口，也是生产环境「鉴权怎么做」「越权怎么防」的必答点。执行计划 W2 就列了「注册/登录/JWT + 网关鉴权过滤器」但一直未落地。
...
- **refresh token 存储选型**（Redis vs DB）→ 缓解：默认 Redis 存 refresh（可设 TTL、易作废），数据库落法作为可选；设计上可讲「Redis 存 refresh + rotation 防重放」。
- **网关透传 userId 被伪造**（下游直接信任 X-User-Id 头）→ 缓解：仅网关注入该头、下游不信任外部传入同名头（覆盖或校验来源）；设计上可讲「内网信任边界 + 网关唯一入口」。
...
- refresh token rotation 沿用已验证的设计，与其他项目同一套 JWT 设计哲学互相印证。
