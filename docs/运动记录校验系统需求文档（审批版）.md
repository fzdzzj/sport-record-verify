...
版本：v2.0（深入版）｜ 状态：**待审批** ｜ 依据：《微服务面试项目方案 v2.x》《面试项目执行计划 v2.2》
说明：本版在 v1.0 基础上补齐**算法细节、字段级表结构、状态机迁移矩阵、事件定义、分布式设计、验收用例**，既是开发依据也是面试弹药库。
...
### 1.2 目标
构建"能判断运动记录是否真实"的后端微服务系统（反作弊风控定位），形成 **提交 → 校验 → 判定 → 申诉 → 榜单沉淀** 完整闭环；作为面试主项目展示微服务架构能力。
...
| 面试准备 | W13-W15 | 故事线、20+ 条 Q&A、mock 面试、缓冲 | 记忆包 |
...
| 外3 Git 托管 | Gitee 私有起步，面试前转公开或共享访问 | 国内访问快 |
| 外4 演示数据 | 公开 GPX 数据集 + 脚本伪造样本 | 真实通过率数据可信 |
| 外5 前端范围 | 单页极简可视化（轨迹地图+记录列表+判定结果），AI 生成 1-2 周 | 演示够用，不抢后端时间 |
| 外6 算法投入 | 接受贯穿项安排 | 已排入 W2-W15 |

**审批结果（2026-04 逐项确认）**：第 1-9、A1、外1、外4、外5 共 13 项按推荐通过 ✅；3 项调整：① A2 算法改考前突击（见 A.4）；② 外2 云服务器**不购买**（本地演示 + 录屏视频）；③ 外3 代码托管改 **GitHub 公开**（要求：代码整洁、README 完善、无敏感信息）。

---

## 附录 A：大厂（一线互联网）Java 面试深度补充

### A.1 定位（诚实）
本项目深度**足以支撑大厂"项目深挖"环节**（4 服务 + 分库分表 + 限流熔断 + 事件 + 灰度 + 自研算法 + 幂等，属电商微服务档且算法差异化）。但大厂面试 **60%+ 分数来自原理八股 + 算法 + 系统设计**，项目是佐证，不是全部。策略：**项目负责深度，八股+算法负责广度，两者缺一不可**。

### A.2 每个技术点的大厂追问清单（实现后必须能答到底）
| 技术点 | 必须答到底的问题 |
| --- | --- |
| MySQL | 为什么 B+ 树；事务隔离级别与 MVCC（当前读/快照读）；分片后跨分片查询怎么办；Explain 慢查询案例 |
| Redis | RDB/AOF 取舍；缓存一致性（先删缓存还是先更库、延迟双删）；Redisson 锁原理（看门狗续期/可重入/公平锁）；雪崩/击穿/穿透 |
| RocketMQ | 三段可靠性（生产/存储/消费）；顺序消息实现；事务消息（半消息+回查）；重试与死信；为什么选它不选 Kafka |
| ShardingSphere | 分片算法（取模/哈希）；扩容迁移；与 MyBatis-Plus 分页的坑 |
| Sentinel | 滑动窗口限流原理；熔断降级策略（慢调用/异常比例） |
| Nacos | 注册中心 CP/AP 取舍；配置推送机制（长轮询） |
| JVM | 项目中实际调优：GC 日志、大对象（轨迹点数组）、堆参数 |
| 并发 | 线程池参数设计（校验引擎/异步落库）；锁升级；AQS |
| 算法 | LeetCode 高频 150 题（独立准备，W2 起每周 2-3 题） |

### A.3 压测与优化实录（大厂最认"真实数据"）
W8 压测方案：自写并发脚本或 JMeter——提交接口 100/500/1000 并发，记录 **P95/QPS/错误率**，定位瓶颈（DB 连接池 / GC / Feign 线程）并优化，产出**优化前后对比**（含至少 1 个 Explain 慢查询案例与 1 个 GC/连接池调优案例）。这些"坑与优化实录"写进 README/ADR，是大厂追问时最有说服力的素材。

### A.4 追加审批项
- A1. 压测标准：W8 产出压测报告（100/500/1000 并发 + P95/QPS + 至少 1 个优化案例）□
- A2. 算法/八股投入（**已确认：考前突击**）：W13-W15 集中突击（高频 150 题 + 八股主题），W2-W12 每周保持 1-2 题热身防生疏；风险提示：算法纯突击风险高，W10 起转入冲刺节奏。
