...
```
设计点：friendship 规范化存储 + 唯一主键从根上消除 A-B/B-A 重复行。
...
| A1 压测 | W8 产出压测报告（100/500/1000 并发 + 至少 1 个优化案例） | 生产环境最认真实数据 |
| A2 算法与基础原理 | 每周 LeetCode 2-3 题 + 基础原理 1 主题（贯穿 15 周） | 高频考点，与项目技术点对齐 |
| 外1 目标画像 | 不锁死单家，按通用后端岗位准备 | 战线长，通用准备收益最大 |
| 外2 云服务器 | 买阿里云轻量应用服务器（约 100 元/年） | 可作对外展示的公网演示链接 |
...
**审批结果（2026-04 逐项确认）**：第 1-9、A1、外1、外4、外5 共 13 项按推荐通过 ✅；3 项调整：① 算法与基础原理投入改为集中复习；② 外2 云服务器**不购买**（本地演示 + 录屏视频）；③ 外3 代码托管改 **GitHub 公开**（要求：代码整洁、README 完善、无敏感信息）。
